...
def get_dataset_val(dataset, seed, transform=None, power=0.0, output="1d"):
   ...
   datasets = {
      ...
      "KMNIST": {
         #"images_train": KMNIST(root="./data/image", train=True, download=True, transform=transform_train),
         #"images_test": KMNIST(root="./data/image", train=False, download=True, transform=transform_test),
         #"diagrams_dir_train": pickle.load(open("./data/KMNIST_D_train_dir.pkl", "rb")),
         #"diagrams_dir_test": pickle.load(open("./data/KMNIST_D_test_dir{}.pkl".format(transform_test_str), "rb")),
         "dim": (28, 28),
         "n_classes": 10
      }
   }
...
def get_dataset(dataset, transform=None, power=0.0, output="1d"):
   ...
   if transform=="Noise":
      ...
      T = RandomPerspective(distortion_scale=power, p=0.5, interpolation=InterpolationMode.BILINEAR)
   elif transform is None:
      T = None
   else:
      raise ValueError("Supported transforms are 'Affine', 'Noise', 'Perspective', 'Rotation'.")
   ...
   datasets = {
      "MNIST": {
         ...
         "images_test": MNIST(root="./data/image", train=False, download=True, transform=transform_test),
         "diagrams_dir_train": pickle.load(open("./data/MNIST_D_train_dir.pkl", "rb")),
         "diagrams_dir_test": pickle.load(open("./data/MNIST_D_test_dir.pkl", "rb")),
         "dim": (28, 28),
         "n_classes": 10
      },
      "KMNIST": {
         ...
         "images_test": KMNIST(root="./data/image", train=False, download=True, transform=transform_test),
         "diagrams_dir_train": pickle.load(open("./data/KMNIST_D_train_dir.pkl", "rb")),
         "diagrams_dir_test": pickle.load(open("./data/KMNIST_D_test_dir.pkl", "rb")),
         "dim": (28, 28),
         "n_classes": 10
      }
   }
   ...
   images_train = datasets[dataset]["images_train"]
   images_test = datasets[dataset]["images_test"]
   diagrams_train = datasets[dataset]["diagrams_dir_train"]
   diagrams_test = datasets[dataset]["diagrams_dir_test"]
   dim = datasets[dataset]["dim"]
   n_classes = datasets[dataset]["n_classes"]

   return images_train, images_test, diagrams_train, diagrams_test, dim, n_classes
