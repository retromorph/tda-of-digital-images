...
from src.fixed_encoders import EncodedFeatureDataset, PersistenceLandscapeEncoder
from src.models.persistence_cnn1d import PersistenceCNN1D
from src.trainer import Trainer


parser = argparse.ArgumentParser(
    description="Persistence Landscape + small 1D CNN classifier",
    usage="uv run python exp/runners/run_persistence_landscape.py --dataset MNIST",
)
...
encoder = PersistenceLandscapeEncoder(
...
model = PersistenceCNN1D(
    ...
    in_channels=args.num_layers,
    base_channels=args.base_channels,
    dropout=args.dropout,
)

print("Data:\t\t {}, idx=[{}], eps={}".format(args.dataset, ", ".join(map(str, args.idx)), args.eps))
print("Encoder:\t PL, num_layers={}, resolution={}".format(args.num_layers, args.resolution))
print("Optimization:\t lr={}, batch size={}, seed={}, device={}".format(args.lr, args.batch_size, args.seed, args.device))

args.task = meta.task
logger = build_mlflow_logger(args, method_name="PL", task_name=args.dataset, model=model)
...
trainer.fit(
    ...
    desc="PL-{}, {}".format(args.dataset, args.seed),
    close_logger=False,
)
